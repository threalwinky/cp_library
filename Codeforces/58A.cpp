...
signed main(){
    ...
    for (int i = 0; i<s.length();i++){
    	if (t.find(s[i]) != string::npos) tmp += s[i]; 
    }
    int check = 0;
    for (int i = 0; i<tmp.length(); i++){
    	if (check == 0 && tmp[i] == 'h') check++;
    	else if (check == 1 && tmp[i] == 'e') check++;
    	else if (check == 2 && tmp[i] == 'l') check++;
    	else if (check == 3 && tmp[i] == 'l') check++;
    	else if (check == 4 && tmp[i] == 'o') check++;
    }
    cout<<((check == 5) ? "YES" : "NO");
}