...
void dfs2(int u){ 
    for (auto x:v[u]){
        if (par[u] != par[x]){
            if (sum1[par[x]] != 0){
                sum1[par[u]] +=sum1[par[x]] + sum[par[x]];
                return;
            }
            dfs2(x);
            sum1[par[u]] += sum[par[x]] + sum1[par[x]];
        }
    }
}
...
signed main(){
    ...
    while (!st.empty()){
        int tmp = st.top();
        if (!check[tmp]){
            sum[tmp] = dfs1(tmp, tmp);
        }
        st.pop();
    }
    memset(check, 0, maxN);
    for (int i=1; i<=n; i++){
        if (sum1[par[i]] == 0) dfs2(par[i]);
    }
    for (int i=1; i<=n; i++)
    {
        if (1 + sum1[par[i]] < sum[par[i]]) cout<<sum[par[i]]<<"\n";
        else 
        cout<<1+ sum1[par[i]]<<"\n"; 
    }
}
