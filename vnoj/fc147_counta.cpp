...
ll supow(ll a, ll b){
    if (b == 1) return a;
    if (b % 2 == 0) return (supow(a, b/2) % mod * supow(a, b/2) % mod) % mod;
    return (supow(a, b/2) % mod * supow(a, b/2) % mod * a) %mod;
}
...
signed main(){
    ...
    int n;
    cin >> n;
    cout << (supow(10, n) - 2 * supow(9, n) + supow(8, n)) % mod;
}
